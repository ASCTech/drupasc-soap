...
try {
	...
	$result = $client->call('selNewsSiteHeadline', $param);
	$err = $client->getError();
	if($err) {
...
function output_block_news_results($result, $maxnews) {
	...
	if($numevents > 0){
		while ($i < $maxnews &&  $i < $numevents){
			...
			echo '</a> - <em>',date('m/d/Y', strtotime($result['data'][$i][2])),'</em>';
			echo '</li>';
			$i++;
		}
		echo "<li class='viewall'><a href='".$base_path."news' target='_top' title='View all news' class='headlineLink' >More News...</a></li>";
	}else{
